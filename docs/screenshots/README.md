# Screenshots — Phase 1 (seeded Moodle)

Script-generated by [`scripts/screenshots/take_screenshots.mjs`](../../scripts/screenshots/take_screenshots.mjs)
(logs in as student1 / teacher1 / admin, captures headlessly at 1400×900 @2x).
**Rerun after any seed change** so these never go stale.

The blog references these files **from this repo** via raw GitHub URLs:

```
https://raw.githubusercontent.com/Jayantkhandebharad/MCP-LMS-OSS/main/docs/screenshots/<file>.png
```

## Student view (`student1`)

### Dashboard — both enrolled courses
![Student dashboard](phase1-dashboard.png)

### "Intro to MCP" course page — three named sections, two pages, quiz
![Course page](phase1-course.png)

### Page activity — the content our MCP resources will serve
![Tools, Resources, and Prompts page](phase1-page.png)

### Quiz landing — attempts with grades, highest-grade method
![Quiz summary](phase1-quiz-summary.png)

### Attempt review — grade 4.00/6.00, per-question weighted marks (1 / 2 / 3)
![Quiz review](phase1-quiz-review.png)

## Teacher view (`teacher1`)

### Grader report — student results per activity
![Grader report](phase1-grader-report.png)

## Admin view

### `mcp_service` external service — the 29 whitelisted functions
![Web service functions](phase1-ws-functions.png)

## Phase 3 — the MCP server in action

### Claude (in Claude Code) takes the quiz through our MCP server: 6/6
Captured manually from a live session — tool calls, Claude's reasoning, and the graded result.
![Claude takes the quiz via MCP](phase3-claude-demo.png)

### The client's MCP panel: our server connected, 9 tools discovered
Note the read-only badges — those are our tool annotations (readOnlyHint) being honored;
`start_quiz` and `submit_quiz_answers` correctly lack them.
![MCP servers panel with moodle connected](phase3-tools-list.png)

## Phase 4 — role-gated tools over Streamable HTTP

Script-generated by `scripts/screenshots/phase4_rbac_shots.mjs` (renders LIVE
`tools/list` responses — start the server with `moodle-mcp --http` first) and
`phase4_render_session.mjs` (renders a real `claude -p` transcript).

### The hero: one server, two callers, different tool lists
![Side-by-side tool lists](phase4-tools-diff.png)

Individual panels: `phase4-tools-student.png` (10 tools), `phase4-tools-teacher.png`
(14, creator tools highlighted).

### Teacher workflow demo — real headless Claude session
whoami → create_course → enrol_student (by email) → publish_course, all live.
![Teacher session transcript](phase4-teacher-demo.png)

## Phase 5 — OAuth 2.1 via Keycloak

Script-generated by `scripts/screenshots/phase5_oauth_shots.mjs` (drives the REAL
Keycloak authorization endpoint via a dynamically-registered client) and
`phase5_connected_shot.mjs` (renders the result of a real OAuth session).

### The Keycloak login page an MCP client's browser flow opens
![Keycloak login](phase5-kc-login.png)

### The consent screen — the user authorizes "Claude Code" for specific scopes
![Keycloak consent](phase5-kc-consent.png)

### Connected with zero secrets in the config (the auth-series payoff)
![OAuth connected](phase5-oauth-connected.png)

### The real Claude Code MCP panel after OAuth login: ✓ Connected, 10 learner tools
Authenticated as student1 via the full OAuth flow — note the read-only badges (our
annotations) and that only the learner tool set is present (RBAC gating over OAuth identity).
![Real OAuth-connected panel](phase5-oauth-connected-real.png)

## Phase 6 — sampling + security

Generated by `scripts/demo/sampling_demo.py` (captures a real sampling round-trip)
+ `scripts/screenshots/phase6_sampling_shot.mjs` (renders it).

### MCP sampling round-trip: server-initiated LLM call over fenced course content
Steps ①② (system rules + real Moodle text fenced as untrusted) are captured live from the
running server; step ③ is a sampling-capable demo client's completion (Claude Code doesn't
support sampling). Shows both the sampling primitive and the prompt-injection guardrail.
![Sampling round-trip](phase6-sampling-demo.png)
